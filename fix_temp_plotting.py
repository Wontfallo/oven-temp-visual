import pandas as pd
import numpy as np
import matplotlib.pyplot as plt # For 2D plots and colormap utilities
...
# --- Mayavi 3D Animated Volumetric Plot with Isosurfaces (Corrected Text Properties) ---
def plot_animated_mayavi_volume_iso(data_df):
    ...
    volumetric_temp_data_0 = griddata(sensor_points_xyz_np, temps_at_time_0,
                                     ...
                                     fill_value=np.nanmin(temps_at_time_0)) 
    ...
    otf.add_point(min_temp_global + (max_temp_global - min_temp_global) * 0.4, 0.02) 
    otf.add_point(min_temp_global + (max_temp_global - min_temp_global) * 0.7, 0.1)  
    otf.add_point(max_temp_global, 0.5)                                      
    vol.module_manager.scalar_lut_manager.lut_mode = 'plasma'

    target_iso_levels = []
    if max_temp_global - min_temp_global > 10:
        if max_temp_global > 30: target_iso_levels.append(round(max(min_temp_global + 5, 30.0),1))
        if max_temp_global > 60: target_iso_levels.append(round(max(min_temp_global + 10, 60.0),1))
        if max_temp_global > 90: target_iso_levels.append(round(max(min_temp_global + 15, 90.0),1))
    target_iso_levels = sorted(list(set(target_iso_levels)))
    if not target_iso_levels and max_temp_global - min_temp_global > 1:
        target_iso_levels.append(round(min_temp_global + (max_temp_global - min_temp_global) * 0.5, 1))
    print(f"Target isosurface contours for animation: {target_iso_levels}")

    iso_surfaces_pipeline_objects = []
    iso_colors = [(0.2, 0.8, 0.8, 0.15), (0.2, 0.8, 0.2, 0.2), (0.8, 0.2, 0.2, 0.25)] 
    safe_init_contour = min_temp_frame0 + (max_temp_frame0-min_temp_frame0)*0.5 if not (np.isnan(min_temp_frame0) or np.isnan(max_temp_frame0) or abs(max_temp_frame0-min_temp_frame0) < 0.01) else min_temp_global

    for i in range(len(target_iso_levels)):
        iso = mlab.pipeline.iso_surface(src, contours=[safe_init_contour], 
                                        opacity=iso_colors[i % len(iso_colors)][3], 
                                        color=iso_colors[i % len(iso_colors)][:3])
        iso_surfaces_pipeline_objects.append(iso)
    ...
    cb.label_text_property.font_size = 8 # Smaller font for colorbar labels
    cb.title_text_property.font_size = 10 # Smaller font for colorbar title
    cb.scalar_bar_representation.position = [0.85, 0.1] 
    cb.scalar_bar_representation.position2 = [0.08, 0.8] # Adjusted width

    axes_actor = mlab.axes(xlabel='X', ylabel='Y', zlabel='Z', 
                           ranges=[0,14,0,14,0,14], color=(0.8,0.8,0.8))
    axes_actor.axes.label_format = '%.0f' # Integer format for axes numbers
    axes_actor.label_text_property.font_size = 5 # Smaller font for axis numbers
    axes_actor.title_text_property.font_size = 7 # Smaller font for axis titles (X,Y,Z)
    ...
    def anim_loop_func():
        for frame_idx in range(num_frames):
            ...
            current_min_sensor_temp_for_fill = np.min(temps_at_time) 
            
            new_volumetric_data = griddata(sensor_points_xyz_np, temps_at_time,
                                           (grid_x_mg, grid_y_mg, grid_z_mg), method='linear',
                                           fill_value=current_min_sensor_temp_for_fill) 
            
            src.mlab_source.scalars = new_volumetric_data
            sensor_viz_points.mlab_source.scalars = temps_at_time 

            for i, iso_pipeline_obj in enumerate(iso_surfaces_pipeline_objects):
                if i < len(target_iso_levels): 
                    iso_pipeline_obj.contour.contours = [target_iso_levels[i]]
...
# --- Static Isosurface Plot (Mayavi - Refined Text Properties) ---
def plot_mayavi_static_isosurfaces(data_df, time_index, num_isosurfaces=4):
    ...
    opacities = np.linspace(0.2, 0.5, len(iso_levels)) # Slightly increased base opacity
    ...
    sensor_plot = mlab.points3d(sensor_x, sensor_y, sensor_z, temps_at_time,
                                scale_mode='none', scale_factor=0.3, # Smaller sensor points
                                colormap='coolwarm', vmin=min_temp_global, vmax=max_temp_global)
    ...
    mlab.show()
# --- 2D Matplotlib Plots (Keep as is) ---
# (The plot_2d_analysis function from the previous response)
...
if __name__ == "__main__":
    # ... (keep the main menu logic as is)
    ...
    if choice == '1':
        ...
        plot_mayavi_static_isosurfaces(df_cleaned, time_index, num_isosurfaces=num_iso) # Changed function name
